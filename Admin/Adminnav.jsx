...
const Adminnav = ({ userName, id }) => {
  return (
    <nav className="navbar navbar-expand-lg bg-success d-flex" style={{ position: 'fixed', right: "10%", top: 150, width: '85%', zIndex: 1000 }}>
        ...
        <div className="collapse navbar-collapse" id="navbarNavDropdown">
          <ul className="navbar-nav d-flex justify-content-between" style={{ width: "100%" }}>
            <li className="nav-item" style={{ width: "16%" }}>
              <Link className="nav-link text-white" to={`/admin/${id}`}>
                Home
              </Link>
            </li>
            <li className="nav-item" style={{ width: "16%" }}>
              <Link to={`/adminbalance/${id}`} className="nav-link text-white">
                Balance
              </Link>
            </li>
            <li className="nav-item" style={{ width: "16%" }}>
              <Link to={`/admindeposit/${id}`} className="nav-link text-white">
                Deposit
              </Link>
            </li>
            <li className="nav-item" style={{ width: "16%" }}>
              <Link to={`/adminwithdraw/${id}`} className="nav-link text-white">
                Withdraw
              </Link>
            </li>
            <li className="nav-item" style={{ width: "16%" }}>
              <Link to={`/admintransfer/${id}`} className="nav-link text-white">
                Transfer
              </Link>
            </li>
            <li className="nav-item" style={{ width: "16%" }}>
              <Link to={`/adminloan/${id}`} className="nav-link text-white">
                Loan
              </Link>
            </li>
            <li className="nav-item" style={{ width: "30%" }}>
              ...
              <path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0Zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4Zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.680-4.168 1.332-.678.678-.83 1.418-.832 1.664h10Z"/>
                ...
                <Dropdown.Menu style={{ zIndex: 1001 }}>
                  <Dropdown.Item href={`/au/${id}`} className="text-success">Users</Dropdown.Item>
                  <Dropdown.Item href={`/admins/${id}`} className="text-success">Settings</Dropdown.Item>
